  ...
  override def extract(spark: SparkSession, webService: WebService): Dataset[String] =
    {
      val totalUrls: Int = math.min({
        val body: String = webService.readOrDefault(FinderExtractor.clusterURL(finderConf, apiBaseUrl, 0, 0), """{"count": 0}""")
        """"count"\s*:\s*(\d+)""".r.findFirstMatchIn(body).get.group(1).toInt
      }, finderConf.vacsLimit) / finderConf.vacsPerPage

      import spark.implicits._

      val clusterUrlsDS: Dataset[String] = (0 to totalUrls)
        .map(value => FinderExtractor.clusterURL(finderConf, apiBaseUrl, finderConf.vacsPerPage, finderConf.vacsPerPage * value))
      ...
      vacancyIdsDF.mapPartitions(part => part.map(row => FinderExtractor.vacancyURL(apiBaseUrl, row.getLong(0))))
        .mapPartitions(part => {
          part.flatMap(url => webService.readOrNone(url))
  ...
  override def filterUnActiveVacancies(spark: SparkSession, idsDF: DataFrame, webService: WebService): DataFrame =
    {
      ...
      idsDF.mapPartitions(part => part.flatMap(row => {
        ...
        val body = webService.readOrDefault(FinderExtractor.vacancyURL(apiBaseUrl, id), """{"status": "pupu"}""")
