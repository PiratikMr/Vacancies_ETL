...
--// FINAL VIEWS
    ...
    */ 
    ...
    drop materialized view if exists pop_employers;
    create materialized view pop_employers as
        with total as (
            select
                v.employer_id,
                count(*) as empl_total,
        ...
        select
            ...
            round(t.empl_total * 100.0 / t.total, 2) as percent
        from total as t
        join employers as e on e.id = t.employer_id
        order by percent desc
        limit 20;
    --//

    --// employers with (highest salary's mediane and most popular):
    --//    case0 (empl: NoName (~0.0001% vac), salary's mediane: inf)
